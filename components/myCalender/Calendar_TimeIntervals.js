...
function Calendar_TimeIntervals({ clicked, isClicked, switchLayout, oneUnit }) {
  ...
  function renderSwitch(switchLayout) {
    switch (switchLayout) {
      case "monthly":
        const current_month = DateTime.fromISO(DateTime.local().toISODate().slice(0, 8) + "01") // initialize to the first DAY of CURRENT month
        ...
        let diff_value = diff_month.toObject()["years"]*12 + diff_month.toObject()["months"]
        isClicked(diff_value )

        return diff_value

      case "weekly":
        const current_week = DateTime.fromISO(DateTime.local().startOf('week').minus({days: 1}).toISODate());
        // 1. current week ->
        // 2. find the start DAY of the week ->
        // 3. minus 1 day becuz LUXON does not start on Sun, but Mon
        const compare_week = DateTime.fromISO(DateTime.fromISO(pickDate).startOf('week').minus({days: 1}).toISODate())
